...
- **Jump-to-section:** `Item N` headings in SEC filings usually carry no id of their own, so the anchor pass synthesises one (`sec-item-...`) and assigns it to the element. That assignment happens on the parsed soup, which must be serialised back out — otherwise the ids exist only in the anchor list and resolve nowhere, and every `Item` entry in the dropdown silently does nothing under lazy loading. Anchors whose target does not exist are also dropped, since filings ship dangling table-of-contents links (Microsoft's 2023 10-K has 14).
